    ...
    <div className="flex flex-col">
      ...
      <div className="grid grid-cols-7 gap-px bg-gray-200">
        {days.map((day, index) => {
          ...
          // SAFELY access "animalType" with bracket notation
          const reservationsSummary = dayReservations.reduce((acc, res) => {
            const type = res["animalType"] as AnimalType || AnimalType.Other;
          ...
          return (
            <div
              key={index}
              className={`relative flex flex-col p-2 bg-white transition-all duration-200 group min-h-[120px] ${
                isCurrentMonth ? 'text-gray-800' : 'text-gray-400 bg-gray-50'
              } ${isSelected(day) ? 'ring-2 ring-cyan-500 z-10' : 'hover:bg-gray-100'} ${
                hasMatch ? 'bg-cyan-50' : ''
              } ${hasSearchTerm && !hasMatch && isCurrentMonth ? 'opacity-50' : ''}`}
              onClick={() => onDateSelect(day)}
            >
              <div className="flex justify-between items-center">
                <span
                  className={`text-sm font-medium ${
                    isToday(day) ? 'bg-cyan-600 text-white rounded-full h-6 w-6 flex items-center justify-center' : ''
                  }`}
                >
                  {day.getDate()}
                </span>
                <button
                  ...
                  className="opacity-0 group-hover:opacity-100 transition-opacity text-cyan-500 hover:text-cyan-700"
                >
              ...
              {isCurrentMonth && Object.keys(reservationsSummary).length > 0 && (
